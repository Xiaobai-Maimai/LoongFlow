...
|------|------------------------|-----------|
| **核心抽象** | 变异-选择进化 | **PES思考范式：Plan-Execute-Summary思考范式** |
| **学习方式** | 任务特定改进 | **跨任务经验积累：新任务在plan阶段会对记忆库中多个不同任务总结的历史数据启动跨任务经验检索** |
| **推理深度** | 有限 | **结构化长程推理：严格按照plan阶段的长程推理方案执行多岛并行探索、交叉编译等操作** |

### 🧠 PES与ReAct有何不同？

PES与ReAct均为智能体思考执行范式，ReAct侧重即时反馈驱动的短程交互，无经验沉淀，适配简单动态任务；而PES侧重长程记忆驱动的结构化闭环，主打经验沉淀与跨任务复用，适配复杂优化、突破局部最优场景。
